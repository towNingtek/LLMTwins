...
def bundle_prompts(task_list: List[str], language: str = "繁體中文") -> Tuple[str, str]:
    ...
    system = (
        ...
        "  • project_b：抽取「執行單位」，即提交或主辦這份文件的政府機關內部單位（處/局/科/室）。"
        "    判斷依據：文件頁首或表頭中獨立出現的單位名稱（如：計畫處、教育處、工務處），"
        "    或承辦人、科長、單位主管等簽章欄位旁的單位資訊。"
        "    不可從計畫執行內容推測單位。只輸出單位名稱，若找不到回傳空字串。"
        "  • project_start_date：抽取「計畫起始日期」。從文件中找到計畫期間/執行期間的起始日期。"
        "    格式統一輸出為 YYYY-MM-DD。若文件使用民國年，請轉換為西元年（民國年+1911）。"
        "    若找不到回傳空字串。"
        "  • project_due_date：抽取「計畫結束日期」。規則同上。若找不到回傳空字串。"
    )
